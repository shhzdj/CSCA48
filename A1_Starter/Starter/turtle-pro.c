...
CmdNode *newCmdNode(char cmd[10], int val) {
   ...
   */
   ...
   if (strcmp(cmd,"loop") == 0 || strcmp(cmd,"left") == 0 || strcmp(cmd,"right") == 0 || strcmp(cmd,"forward") == 0 || strcmp(cmd,"backward") == 0 || strcmp(cmd,"penup") == 0 ||strcmp(cmd,"pendown") == 0 || strcmp(cmd,"colour") == 0){
...
void insertCommandLoop(CmdNode* head, CmdNode* new_loop){
	/*
	insert a new loop to the list 
	*/
	
	CmdNode* p = NULL;
	// check to see if the linked list has a loop_head
	if(head != NULL) {
		if (head -> loop_head == NULL){
			head -> loop_head = new_loop;
		}
		else{
			p = head-> loop_head;
			while(p -> next != NULL){
   ...
   */
   ...
   while(temp != NULL){
	   if(strcmp(temp->cmd,"penup") == 0 ||strcmp(temp->cmd,"pendown") == 0 || strcmp(temp->cmd,"left") == 0 || strcmp(temp->cmd,"right") == 0 ){
		   ...
		   loop = temp -> loop_head;
		   while(loop != NULL){
			   if(strcmp(loop->cmd,"penup") == 0 ||strcmp(loop->cmd,"pendown")	 == 0 || strcmp(loop->cmd,"left") == 0 || strcmp(loop->cmd,"right") == 0 ){
				   printf("%3d:    %s\n", i, loop->cmd);
				}
				else{
					printf("%3d:    %s %d\n", i, loop->cmd, loop->val);
...
CmdNode *deleteCommandList(CmdNode *head) {
   ...
   */
	...
	CmdNode* tr = NULL;
	CmdNode* loop = NULL;
	CmdNode* tr_loop = NULL;
	
	pre = head;
	while(pre != NULL){
		if (strcmp(pre-> cmd ,"loop") == 0){
			loop = pre-> loop_head;
		...
		if(direction < 0){
		  ...
		  color = head -> val;
			...
			if( x >= 0 && x < SIZEX && y >= 0 && y< SIZEY){
				...
				int i = 0;
				while( i< head-> val){
					CmdNode* loop = head -> loop_head;
					
					while(loop != NULL){
						if(strcmp(loop -> cmd, "pendown") == 0){
							pen = 1;
					
						}
				   
						else if(strcmp(loop -> cmd, "penup") == 0){ 
						  pen = 0;
						}
						
						else if (strcmp(loop-> cmd, "right")== 0){
							direction = (direction + 1)%4;
						}
						else if (strcmp(loop->cmd , "left") == 0){
							direction = (direction - 1) %4;
						}
						if(direction < 0){
							direction = direction + 4;
						  ...
						  color = loop -> val;
							  ...
							  int x = pos_x + (loop -> val * directions[direction][0]);
							  int y = pos_y + (loop -> val * directions[direction][1]);
